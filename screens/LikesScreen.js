...
const LikesScreen = ({ navigation, route }) => {
    ...
    const fetchUsersWhoLiked = async (searchString) => {
        try {
            const docRef = doc(db, 'Posts', placeId);
            const docSnap = await getDoc(docRef);
            const {likes} = docSnap.data();
            console.log(likes);
            console.log("Fetched users that liked the post!");
            ...
            <View>
              ...
              <FormInput
                onChangeText = {(searchString) => (
                    fetchUsersWhoLiked(searchString)
                )}
                  placeHolderText = "Search users who liked the post"
                  iconType = "search1"
                  autoCapitalize = "none"
                  autocorrect = {false}
              />
